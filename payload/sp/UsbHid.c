...
static bool UsbHid_onDeviceAdd(const UsbDeviceInfo *info) {
    ...
    Usb_ctrlTransfer(info->id, requestType, USB_GET_DESCRIPTOR, DESCRIPTOR_TYPE_HID << 8,
            info->interfaceDescriptor.interfaceNumber, 0x9, descriptorBuffer);

    u16 reportDescriptorSize = read_u16_le(descriptorBuffer, 0x7);
    if (reportDescriptorSize > 0x1000) {
        return false;
    }
    Usb_ctrlTransfer(info->id, requestType, USB_GET_DESCRIPTOR, DESCRIPTOR_TYPE_REPORT << 8,
